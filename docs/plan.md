...
| C13 | `ScopeVoter` + `#[IsGranted('SCOPE_x')]`-style checks and an `is_granted_scope()` expression function. Scopes are read from the `scope` claim through a `ProvidesScopes` user, so they answer in `claims` and `custom` modes and are refused in `provider` mode, where the store is the authority | Symfony voters | T2 |
