...
- **Both hops are HTTPS.** A plaintext `discovery` is refused when the container is built. A
  `jwks_uri` that comes back plaintext is refused when it arrives, because that one is the
